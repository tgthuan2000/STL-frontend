...
const BudgetDetailCategory = () => {
    ...
    const {
        data: { budget },
        refetch,
    } = useBudgetDetail(GET_BUDGET_CATEGORY_DETAIL_SPENDING_BY_MONTH)

    return (
        <Transaction hasBack title={t(LANGUAGE.BUDGET_BY_CATEGORY)}>
            <BudgetDetailContent type='categorySpending' data={budget.data} loading={budget.loading} reload={refetch} />
        </Transaction>
    )
}
